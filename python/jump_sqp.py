...
prb.createResidual("min_h_ang", 1000.0 * cs.sumsqr(h))

q_ref = q_init
q_ref[2] += 0.2
prb.createResidual("postural_xy", 1e5 * cs.sumsqr(q[0:2] - q_init[0:2]))
...
prb.createResidual("postural", 1e3 * cs.sumsqr(q[7:-1] - q_init[7:-1]))

# prb.createResidualFunction("min_qdot", 10.*cs.sumsqr(qdot), nodes=list(range(0, lift_node)))
prb.createResidual("min_qddot", 0.2 * cs.sumsqr(qddot), nodes=list(range(0, ns)))
# prb.createResidual("min_f", 0.001 * cs.sumsqr(foot_forces), nodes=list(range(0, ns)))
